...
describe("OneTwoThree Game", async () => {
  ...
  it("cannot enter above 4", async () => {
    await expect(game.connect(accounts[1]).inc(4)).to.revertedWith("you can enter number 1 ~ 3")
  });

  it("cannot enter less 1", async () => {
    await expect(game.connect(accounts[1]).inc(0)).to.revertedWith("you can enter number 1 ~ 3")
  });
  ...
  it("Lose above 30", async () => {
    ...
    await expect(game.connect(accounts[2]).inc(3)).to.be.revertedWith("the game was over. plrease create a new game.")
  });
});
